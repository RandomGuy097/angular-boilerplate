...
export class EndpointContext {
  ...
  public async init(options?: {
    ...
    if (
      ...
      Helpers.throwError(
        `[leo-config] Your${this.host ? ' remote' : ''} 'host' must start with http:// or https://`,
      );
    }
    ...
    if (!this.mode && !this.config.abstract) {
      ...
      Helpers.error(
        `[leo][Context=${this.contextName}]: ${errMsg}`,
        false,
        true,
      );
      //#region @backend
    ...
    if (this.config.abstract) {
      this.logFramework &&
        Helpers.info(
          `[leo] Create abstract context: ${this.config.contextName}`,
        );
    } else {
      if (this.isRemoteHost) {
        this.logFramework &&
          Helpers.info(
            `[leo] Create context for remote host: ${this.config.host}`,
          );
      } else {
        this.logFramework &&
          Helpers.info(`[leo] Create context for host: ${this.config.host}`);
      }
    }
    //#endregion
  ...
  private getAutoGeneratedConfig(): Models.DatabaseConfigTypeOrm {
    ...
    if (this.isRunningInsideDocker) {
      ...
      this.logFramework &&
        Helpers.info(
          `[leo][database] Automatically resolving database config for mode ${this.mode}`,
        );
      switch (this.mode) {
        ...
        case 'backend-frontend(websql)':
          ...
          keepWebsqlDbDataAfterReload =
            LeoAdminService.Instance?.keepWebsqlDbDataAfterReload; // TODO @LAST
          //#endregion
      ...
      return await new Promise(resolve => {
        if (this.isRunningInsideDocker) {
          ...
          this.serverTcpUdp.listen(Number(this.uriPort), '0.0.0.0', () => {
            ...
            this.logFramework &&
              Helpers.log(`[leo][express-server]listening on port: ${this.uriPort}, hostname: ${this.uriPathname},
  ...
  private async createInstances(classes: any, classType: Models.ClassType) {
    ...
    for (const classFn of [
      ...
      const instance = DIContainer.resolve(classFn as any) as any;
  ...
  async initClasses(): Promise<void> {
    ...
    for (const classTypeName of [
      ...
      // Helpers.taskStarted(
      //   `[leo] REINITING _ INS FN ${classTypeName} ${this.contextName} STARTED`,
      // );
      ...
      // Helpers.taskStarted(
      //   `[leo] REINITING _ INS FN ${classTypeName} ${this.contextName} DONE`,
      // );
      //#endregion
    }
  }
  ...
  async initEntities() {
    ...
    for (const entity of entities) {
      ...
      if (createTable) {
        this.logDb &&
          console.info(
            `[leo][typeorm] create table for entity "${nameForEntity}" ? '${createTable}'`,
          );
  ...
  async initDatabaseConnection(): Promise<void> {
    ...
    if (this.modeAllowsDatabaseCreation && this.databaseConfig) {
      this.logDb &&
        this.logFramework &&
        Helpers.info('[leo][database] prepare typeorm connection...');
      try {
        ...
        console.error(
          `[leo][typeorm] Error while initializing connection for ${this.contextName}, ERROR STARTED `,
        );
        console.error(error?.stack || '< No stack trace > ');
        console.error(error?.message || error);
        console.error(
          `[leo][typeorm] Error while initializing connection for ${this.contextName}, ERROR ENDS `,
        );
      }
      ...
      if (this.logDb || this.logFramework) {
        ...
        [leo][typeorm] db prepration done.. db initialize=${this.connection?.isInitialized}
      ...
      Helpers.info(`[leo][typeorm] Not initing db for mode ${this.mode}`);
    }
    //#endregion
  }
  ...
  private updateCalculatedPathsForControllers(
    ...
    const parentsCalculatedPath = _.slice(rawConfigs, 1)
      .reverse()
      .map(bc => {
        if (LeoHelpers.isGoodPath(bc.path)) {
          return bc.path;
        }
        return bc.className;
      })
    ...
    if (LeoHelpers.isGoodPath(classConfig.path)) {
    ...
    for (const controllerClassFn of allControllers) {
      ...
      if (!UtilsOs.isRunningInCliMode()) {
        ...
        this.logHttp &&
          console.groupCollapsed(
            `[leo][express-server] routes [${classConfig.className}]`,
          );
        //#region @backend
      }
      //#endregion
      ...
      for (const methodName of methodNames) {
        ...
        const expressPath = methodConfig.global
          ...
          : LeoHelpers.getExpressPath(classConfig, methodConfig);
  ...
  public writeActiveRoutes(): void {
    ...
    const troutes = Utils.uniqArray(
      ...
      return (
        ...
        LeoHelpers.fillUpTo(method.toUpperCase() + ' ', 10) +
        this.uriOrigin +
        expressPath
      );

      // return `${LeoHelpers.string(method.toUpperCase() + ':')
    ...
    this.logFramework && console.log(`[leo] routes file: ${fileName} `);
  ...
  private async initBackendMiddlewares(): Promise<void> {
    ...
    if (this.session) {
      Helpers.info(
        '[leo][express-server] session enabled for this context ' +
          this.contextName,
      );
      ...
      this.logHttp &&
        Helpers.info(
          `[leo][express-server] session not enabled for this context '${this.contextName}'`,
        );
      // }
      app.use(
        cors({
  ...
  private initServer(
    ...
    if (UtilsOs.isElectron) {
      //#region @backend
      const ipcKeyName = LeoHelpers.ipcKeyNameRequest(
        target,
        methodConfig,
        expressPath,
      );
      ipcMain.on(ipcKeyName, async (event, paramsFromBrowser) => {
        ...
        const sendToIpsMainOn = LeoHelpers.ipcKeyNameResponse(
          target,
          methodConfig,
          expressPath,
        );
        ...
        async (req: expressType.Request, res: expressType.Response) => {
          ...
          if (req.headers[Symbols.old.CIRCURAL_OBJECTS_MAP_BODY]) {
            try {
              tBody = JSON.parse(
                ...
                JSON.parse(
                  LeoHelpers.firstStringOrElemFromArray(
          ...
          if (req.headers[Symbols.old.MAPPING_CONFIG_HEADER_QUERY_PARAMS]) {
            try {
              ...
              tQuery = LeoHelpers.parseJSONwithStringJSONs(
            ...
            Object.keys(tQuery).forEach(queryParamName => {
              try {
                ...
                if (_.isString(beforeTransofrm)) {
                  try {
                    const paresed =
                      LeoHelpers.tryTransformParam(beforeTransofrm);
                ...
                tQuery[queryParamName] =
                  LeoHelpers.parseJSONwithStringJSONs(afterEncoding);
              } catch (e) {}
            });
          }
          ...
          const resolvedParams = args
            .reverse()
            .map(v => LeoHelpers.tryTransformParam(v));
    ...
    if (UtilsOs.isElectron) {
      ...
      target.prototype[methodConfig.methodName] = function (...args) {
        const received = new Promise(async (resolve, reject) => {
          const headers = {};
          const { request, response } = LeoHelpers.websqlMocks(headers);

          //#region @browser
          ipcRenderer.once(
            LeoHelpers.ipcKeyNameResponse(target, methodConfig, expressPath),
            (event, responseData) => {
          ...
          ipcRenderer.send(
            LeoHelpers.ipcKeyNameRequest(target, methodConfig, expressPath),
            args,
          );
          //#endregion
        });
