        ...
        setTimeout(async () => {
          if (UtilsOs.isRunningInDocker()) {
            ...
            if (
              ...
              console.warn(
                `[leo] Context ${endpointContextRef.contextName} is not active context, skipping initialization.`,
              );
          ...
          if (endpointContextRef.databaseConfig) {
            ...
            keepWebsqlDbDataAfterReload =
              LeoAdminService.Instance?.keepWebsqlDbDataAfterReload;

            if (keepWebsqlDbDataAfterReload) {
              !UtilsOs.isRunningInCliMode() &&
                Helpers.info(
                  `[leo] Keeping websql data after reload ` +
                    `(context=${endpointContextRef.contextName}).`,
                );
            } else {
              Helpers.info(
                `[leo] Dropping all tables and data ` +
                  `(context=${endpointContextRef.contextName}).`,
              );
            }
            //#endregion
            ...
            if (endpointContextRef.onlyMigrationRun) {
              endpointContextRef.logMigrations &&
                Helpers.log(
                  `[leo] Running only migrations (context=${endpointContextRef.contextName}).`,
                );
              ...
              endpointContextRef.logMigrations &&
                Helpers.log(
                  `[leo] Reverting migrations to timestamp ${
              ...
              endpointContextRef.logMigrations &&
                Helpers.log(
                  `[leo] Running all not applied migrations (context=${endpointContextRef.contextName}).`,
                );
              await endpointContextRef.dbMigrations.runAllNotCompletedMigrations();
            }
            //#endregion
